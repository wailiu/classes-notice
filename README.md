...
- 课时包/报名:购买课时、剩余课时、有效期,报名时同步生成缴费流水;支持手动调整课时(赠课/纠错)
...
- 家长端:多孩子切换、剩余课时与课时包明细、未来两周可约课表(余位/已满/已约状态)、预约、取消预约、预约记录
- 老师端:未来两周所教课次(预约人数)、课次学员名单(含家长电话,可拨打)、帮学员签到

**核心业务规则(服务端强校验)**
- 预约:课次未开始、学员在读、不可重复预约、班级容量未满、同一时段无冲突、自动挑选可用课时包(优先科目专属、先到期先用),预约即扣 1 课时(事务 + 行锁防超扣)
...
| mini | mock 码 `mock-parent-1` | 家长「张爸爸」,名下两个孩子 |
| mini | mock 码 `mock-parent-2` | 家长「刘妈妈」 |
...
```bash
cd server
npm test    # 预约/课时扣减/容量/冲突/取消规则 9 个用例
```

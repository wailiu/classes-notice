...
- 家长端:多孩子切换、剩余课时与课时包明细、**按课种 → 选时段 → 单次多选/长期预约**的完整预约流程(见下文)、取消预约、预约记录。**只能预约已购买课种**:未购买或该课种课时不足的课种不可进入预约,并展示"未购买该课种 / 该课种课时不足"等中文原因
- 老师端:未来两周所教课次(预约人数)、课次学员名单(含家长电话,可拨打)、帮学员签到

**家长预约流程(课种 → 时段 → 课次)**

预约不是在课表上点一下:很多课种同一周有多个固定时段(如素描每周六、日上/下午各一节),家长需要先选课种、再选上课时段,然后决定约哪几次课:

1. **预约课程列表页**:按课种展示,**已购买且有剩余课时的课种置顶**(排序由服务端完成),卡片展示课种名、剩余课时、全部时段摘要(周六上午 10:00-11:30 等);未购买课种置底、不可点入,提示联系前台报名
2. **选时段**:进入已购课种后,列出该课种全部可预约时段(来自班级排课模板:周几 + 上/下午 + 时间 + 老师 + 教室),12:00 前为上午、18:00 前为下午
3. **单次预约**:勾选该时段未来若干次课(可多选、可全选),提交前确认将约几节、扣几课时
4. **长期预约**:选「未来两周 / 未来一个月」,一键预约该时段范围内全部课次(例:一次性预约未来一个月每周六上午的素描课);提交前预览共几节、可约几节、不可约的自动跳过原因
5. **结果反馈**:服务端返回成功课次、失败课次及原因(已满/已约/时间冲突)、实际扣课时数;**课时不足以覆盖全部可约课次时整批拒绝、不扣任何课时**,绝不超扣
...
```bash
cd server
npm test    # 37 个用例:预约/课时扣减/容量/冲突/取消 + 课种匹配(跨课种拒绝、通用包禁用)
            # + 课种列表已购置顶排序 + 时段/课次标记 + 批量/长期预约(部分成功、课时不足不超扣、未购拒绝)
```
...
└── mini/                     # 原生微信小程序
    └── pages/                # 登录/家长(首页/预约课种列表/选时段预约/记录)/老师(首页/名单)/未绑定
```
